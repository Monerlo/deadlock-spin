...
// Компонент для іконки-стрілки (шеврон)
const ChevronIcon = () => (
  <svg 
...
const HeroCardComponent = ({ hero, isSelected, onClick, priority = null }: HeroCardProps) => {
  ...
  // 1. Пріоритет має найвищий пріоритет
  if (priority) {
    switch (priority) {
      case 3: borderClass = 'border-[#8A2BE2]'; break; // Фіолетовий
      case 2: borderClass = 'border-yellow-400'; break;   // Яскраво-жовтий
      case 1: borderClass = 'border-[#808080]'; break; // Сірий
    }
  // 2. Якщо немає пріоритету, але герой обраний, показуємо стандартну рамку вибору
  } else if (isSelected) {
    borderClass = 'border-[#C09B54]'; // Золотий
  }
  ...
  return (
    <div
      ...
      className={`
        relative ${baseClasses} ${borderClass} ${selectionClasses} ${hoverClasses}
        cursor-pointer w-24 h-24 sm:w-28 sm:h-28 flex items-center justify-center overflow-hidden
      `}
      title={hero.name}
    >
      <img
